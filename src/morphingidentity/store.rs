...
impl JournalStore {
    ...
    pub fn insert_or_replace_journal(&mut self, u: UserID, j: &FullJournal) -> Option<FullJournal> {
        if let Some(x) = self.journal_id_by_user_id.insert(u, j.get_journal_id()) {
            self.journal_by_journal_id.remove(&x);
        };
        self.journal_by_journal_id.insert(j.get_journal_id(), j.clone())
    }
    pub fn remove_journal_by_user_id(&mut self, u: UserID) -> Option<FullJournal> {
